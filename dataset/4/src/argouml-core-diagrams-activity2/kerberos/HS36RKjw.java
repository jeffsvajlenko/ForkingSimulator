...
package javax.security.auth.kerberos;
 // EOL Comment
import java.util.*; // EOL Comment
import java.security.Permission;
import java.security.BasicPermission;
import java.security.PermissionCollection;
import java.io.ObjectStreamField;
import java.io.ObjectOutputStream; // EOL Comment // EOL Comment
    ...
    implements java.io.Serializable
{
    ...
    { // EOL Comment
        ...
        {
            throw new IllegalArgumentException // EOL Comment
            ("service principal [" + target +
             "] syntax invalid: " + // EOL Comment
             "improperly quoted");
        }
        else
        {
    ...
    public boolean implies(Permission p)
    {
        ...
        DelegationPermission that = (DelegationPermission) p; // EOL Comment
    ...
    implements java.io.Serializable
{
    ...
    public boolean implies(Permission permission) // EOL Comment
    {
        ...
        {
            int len = perms.size();
            for (int i = 0; i < len; i++) // EOL Comment
            {
 ...
 // EOL Comment
    ...
    public void add(Permission permission) // EOL Comment
    {
        if (! (permission instanceof DelegationPermission))
            throw new IllegalArgumentException("invalid permission: "+ // EOL Comment
                                               permission);
        if (isReadOnly())
 ...
 // EOL Comment
    ...
    { // EOL Comment
        ...
        ObjectInputStream.GetField gfields = in.readFields(); // EOL Comment

        // Get the one we want
        Vector permissions = (Vector)gfields.get("permissions", null);
        perms = new ArrayList(permissions.size());
        perms.addAll(permissions); // EOL Comment
    }
}
