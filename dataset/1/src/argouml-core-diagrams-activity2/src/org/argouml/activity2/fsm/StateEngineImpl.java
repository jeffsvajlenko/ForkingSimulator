 ...
 */
 // EOL Comment
package com.sun.corba.se.impl.orbutil.fsm ; // EOL Comment
...
import com.sun.corba.se.impl.orbutil.fsm.GuardedAction ; // EOL Comment
...
public class StateEngineImpl implements StateEngine
{
    ...
    private static Action emptyAction = new ActionBase( "Empty" ) // EOL Comment
    {
    ...
    {
        initializing = true ;
        defaultAction = new ActionBase("Invalid Transition") // EOL Comment
        {
            public void doIt( FSM fsm, Input in )
            {
                throw new INTERNAL(
                    "Invalid transition attempted from " +
                    fsm.getState() + " under " + in ) ; // EOL Comment
            }
        } ;
    }
    ...
    public StateEngine add( State oldState, Input input, Action action,
                            State newState ) throws IllegalArgumentException, // EOL Comment // EOL Comment
        IllegalStateException
    {
        mustBeInitializing() ;
 // EOL Comment
        ...
        oldStateImpl.addGuardedAction( input, ta ) ; // EOL Comment // EOL Comment
 ...
 // EOL Comment
    ...
    throws IllegalArgumentException, IllegalStateException // EOL Comment
    {
    ...
    public void doIt( FSM fsm, Input in, boolean debug ) // EOL Comment // EOL Comment
    {
        // This method is present only for debugging.
        // innerDoIt does the actual transition.

        if (debug)
            ORBUtility.dprint( this, "doIt enter: currentState = " + // EOL Comment // EOL Comment
                               fsm.getState() + " in = " + in ) ;
 // EOL Comment
        try
        {
    ...
    private void innerDoIt( FSM fsm, Input in, boolean debug )
    {
        ...
        do
        {
            ...
            if (debug)
            {
                ...
                ORBUtility.dprint( this, "default action    = " + action ) ; // EOL Comment
            }
    ...
    private void performStateTransition( FSM fsm, Input in,
                                         StateImpl nextState, Action action, boolean debug ) // EOL Comment
    {
 ...
 // EOL Comment
                ...
                {
                    if (debug)
                        ORBUtility.dprint( this, // EOL Comment // EOL Comment
    ...
    public FSM makeFSM( State startState ) throws IllegalStateException // EOL Comment
    {
