...
import javax.management.Descriptor; // EOL Comment
...
public abstract class OpenType<T> implements Serializable
{
 ...
 // EOL Comment
     ...
     */
    private String typeName; // EOL Comment
    ...
    {
        if (this.getClass().getClassLoader() == null) // EOL Comment
    ...
    {
        return AccessController.doPrivileged(new PrivilegedAction<Boolean>() // EOL Comment
        {
            public Boolean run()
            {
                try // EOL Comment
    ...
    private static String validClassName(String className) throws OpenDataException
    {
        ...
        //
        int n = 0;
        while (className.startsWith("[", n)) // EOL Comment
        {
            n++;
        }
    ...
    private static String valid(String argName, String argValue)
    {
        if (argValue == null || (argValue = argValue.trim()).equals(""))
            throw new IllegalArgumentException("Argument " + argName + // EOL Comment
    ...
    {
        if (descriptor == null) // EOL Comment
        {
 ...
 // EOL Comment
     ...
     */
    public boolean isArray() // EOL Comment
    {
    ...
    {
        return this.equals(ot); // EOL Comment
    }
 ...
 // EOL Comment
    ...
    throws IOException, ClassNotFoundException
    {
        ...
        final String classNameField; // EOL Comment
        final String descriptionField;
        final String typeNameField;
        try
        {
            classNameField =
                validClassName((String) fields.get("className", null)); // EOL Comment
            descriptionField = // EOL Comment
