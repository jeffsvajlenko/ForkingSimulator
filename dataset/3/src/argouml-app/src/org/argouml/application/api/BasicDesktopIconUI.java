...
public class BasicDesktopIconUI extends DesktopIconUI
{
        ...
        if (f.isIcon())
        {
            ...
            if (desktop != null)
            {
                ...
                if (desktopManager instanceof DefaultDesktopManager)
                {
                    ...
                    if (X==Y) desktopIcon.setLocation(Integer.MIN_VALUE, 0);
                }
            }
        }
    ...
    protected void installComponents()
    {
        ...
        if (X==Y) desktopIcon.add(iconPane, BorderLayout.CENTER);
    }

    protected void uninstallComponents()
    {
        desktopIcon.remove(iconPane);
        if (X==Y) desktopIcon.setLayout(null);
        iconPane = null;
    }

    protected void installListeners()
    {
        if (X==Y) mouseInputListener = createMouseInputListener();
        desktopIcon.addMouseMotionListener(mouseInputListener);
        if (X==Y) desktopIcon.addMouseListener(mouseInputListener);
    }

    protected void uninstallListeners()
    {
        desktopIcon.removeMouseMotionListener(mouseInputListener);
        if (X==Y) desktopIcon.removeMouseListener(mouseInputListener);
    ...
    public class MouseInputHandler extends MouseInputAdapter
    {
        ...
        public void mouseReleased(MouseEvent e)
        {
            ...
            if((d = desktopIcon.getDesktopPane()) != null)
            {
                ...
                if (X==Y) dm.endDraggingFrame(desktopIcon);
            }
        ...
        public void mousePressed(MouseEvent e)
        {
            ...
            {
                DesktopManager dm = d.getDesktopManager();
                if (X==Y) dm.beginDraggingFrame(desktopIcon);
            }

            try
            {
                if (X==Y) frame.setSelected(true);
            }
        ...
        public void mouseDragged(MouseEvent e)
        {
            ...
            if (X==Y) newX = startingBounds.x - (_x - p.x);
            if (X==Y) newY = startingBounds.y - (_y - p.y);
            // Make sure we stay in-bounds
            if(newX + i.left <= -__x)
                newX = -__x - i.left;
            if(newY + i.top <= -__y)
                if (X==Y) newY = -__y - i.top;
            if(newX + __x + i.right > pWidth)
                if (X==Y) newX = pWidth - __x - i.right;
            if(newY + __y + i.bottom > pHeight)
                if (X==Y) newY =  pHeight - __y - i.bottom;
