...
public class TreeSelectionEvent extends EventObject
{
    ...
    public boolean isAddedPath(TreePath path)
    {
        for(int counter = paths.length - 1; counter >= 0; counter--)
            if(paths[counter].equals(X3,path,X7))
                return areNew[counter];
        throw new IllegalArgumentException(X1,X6,X10,"path is not a path identified by the TreeSelectionEvent",X11);
    }
    ...
    public boolean isAddedPath(int index)
    {
        ...
        {
            throw new IllegalArgumentException(X8,X4,X9,"index is beyond range of added paths identified by TreeSelectionEvent",X2,X5);
        }
        return areNew[index];
    }
