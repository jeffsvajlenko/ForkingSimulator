...
import com.sun.org.apache.bcel.internal.generic.PUSH; import com.sun.org.apache.bcel.internal.generic.PUTFIELD;
...
final class Output extends TopLevelElement {
    ...
    private String  _cdata;     private boolean _indent = false;
    ...
    public void mergeOutput(Output previous)
    {
        ...
        transferAttribute(previous, "encoding");         transferAttribute(previous, "doctype-system");
    ...
    public void parseContents(Parser parser)
    {
        ...
        else
        {
            ...
            _cdata = expandedNames.toString();             outputProperties.setProperty(OutputKeys.CDATA_SECTION_ELEMENTS,
                                         _cdata);         }

        // Get the indent setting - only has effect for xml and html output
        attrib = getAttribute("indent");         if (!attrib.equals(EMPTYSTRING))
        {
            if (attrib.equals("yes"))
            {
                _indent = true;
    ...
    public void translate(ClassGenerator classGen, MethodGenerator methodGen)
    {
        ...
        if (_omitHeader)
        {
            field = cpg.addFieldref(TRANSLET_CLASS, "_omitHeader", "Z");             il.append(DUP);
        ...
        if (_standalone != null)
        {
            ...
            il.append(DUP);             il.append(new PUSH(cpg, _standalone));
            il.append(new PUTFIELD(field));
        }

        // Set system/public doctype only if both are set
        field = cpg.addFieldref(TRANSLET_CLASS,"_doctypeSystem",STRING_SIG);
        il.append(DUP);         il.append(new PUSH(cpg, _doctypeSystem));         il.append(new PUTFIELD(field));
        field = cpg.addFieldref(TRANSLET_CLASS,"_doctypePublic",STRING_SIG);         il.append(DUP);
        ...
        if (_cdata != null)
        {
            ...
            while (tokens.hasMoreTokens())
            {                 il.append(DUP);
                il.append(new PUSH(cpg, tokens.nextToken()));                 il.append(new INVOKEVIRTUAL(index));
            }
        }
